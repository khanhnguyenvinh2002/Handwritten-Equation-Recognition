...
image_list = glob.glob("./test-images/*.png")
...
for im_name in image_list:
    ...
    # imgrey = cv2.blur(imgrey, (11, 11))
    
    # blurred = cv2.GaussianBlur(imgrey, (5, 5), 0)
    # value, thresh = cv2.threshold(blurred, 60, 255, cv2.THRESH_BINARY_INV)
    # blurred = cv2.GaussianBlur(imgrey, (5, 5), 0)
    value, thresh = cv2.threshold(imgrey, 127, 255, 0)
    # thresh = cv2.threshold(imgrey, 60, 255, cv2.THRESH_BINARY)[1]
    ...
    # # imgrey = cv2.blur(imgrey, (11, 11))
    # thresh = cv2.threshold(imgrey, 60, 255, cv2.THRESH_BINARY)[1]
    # contours, hierachy = cv2.findContours(thresh, 1,2) # RETR_EXTERNAL for only bounding outer box
